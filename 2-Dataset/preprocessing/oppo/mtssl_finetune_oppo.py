...
FOLDER = SCRATCH + '/mp-dataset/opportunity/OpportunityUCIDataset/'
...
OUTDIR = SCRATCH + f"/mp-dataset/finetune_opportunity_{TARGET_HZ}hz_w{WINDOW_SEC}/"
...
def main():
    ...
    for file_path in tqdm(DATAFILES):
        ...
        p_id = int(file_path.split("/")[-1][1:2])
        ...
        current_X, current_y = content2x_and_y(
            datContent, sample_rate=sample_rate, epoch_len=WINDOW_SEC, overlap=WINDOW_OVERLAP_SEC
        )
    ...
    Y = Y.flatten()
    Y = Y.astype(float) # converts string to float
    Y = Y.astype(int) # converts float to integer
    X = X / 1000  # convert to unit of g
    clip_value = 3
    ...
    Y[Y == 5] = 3
    label_map = np.vectorize(LABEL_NAMES.get)
    # Apply the function to your Y array
    Y = label_map(Y)
    FILTERED_LABEL= []
