...
def label_mapto_capture24(Y, FILTERED_LABEL, walmsley2020=False, willetts2018=False):
    ...
    mapping =   {
        'high_resistance_bike': 'bicycling'
        ,'low_resistance_bike': 'bicycling'
        ,'run': 'sports'
        ,'walk': 'walking'
                }
...
def read_records(records_file):
    ...
    records_lst=[x.decode('utf-8') for x in records]
    return records_lst

def read_wristppg_wfdata(filesetname,channels):
    record = wfdb.rdrecord(filesetname,channel_names=channels)
    wf_dat = record.__dict__
    return wf_dat
