...
echo "'high_resistance_bike','low_resistance_bike','run','walk'"
echo "--------------------"
...
script="$ABSOLUTE_PATH/2-Dataset/preprocessing/wristppg/mtssl_finetune_wristppg.py"

case $choice in 
    [yY] )
        job_name="ftdat-wristppg-relabel"
        relabel="--relabel"
        ;;
    [nN] )
        job_name="ftdat-wristppg-adl"
        relabel=""
        ;;
esac
