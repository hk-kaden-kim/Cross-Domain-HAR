"""
Create MTSSL finetuning data from the ichi14 dataset.
...
import os
import glob
from utils import resize, is_good_quality, label_mapto_capture24, extract_number, label_sleep
...
FOLDER = os.path.join(SCRATCH, 'mp-dataset', 'ichi14', 'data')
...
OUTDIR = SCRATCH + f"/mp-dataset/finetune_ichi14_{TARGET_HZ}hz_w{WINDOW_SEC}/"
...
def main():
    ...
    total_samples = 0
    # combine_files()
    print("Create MTSSL Finetune dataset of Mendeley Data, (ichi14) dataset from study 'A multi-sensory dataset for the activities of daily living'...")
    cnt, cnt_nanlabel_window = 0, 0
    for filename in glob.glob(DATAFILES.format(FOLDER)):
        if filename in FILTERED_FILES:
         continue
        # Remove the file extension
        elif filename in COMBO:
            data1 = np.load(filename, allow_pickle = True)
            match = re.search(r'p\d+a', filename)
            if match:
                # replace 'a' with 'b' in the matched substring
                new_substring = match.group().replace('a', 'b')
                # replace the matched substring with the new substring in the filename
                filename2 = filename.replace(match.group(), new_substring)
        ...
        if len(df) >= WINDOW_LEN:
            for i in range(0, len(df), WINDOW_STEP_LEN):
                ...
                label_name = df_window['gt'].mode(dropna=True).values
    ...
    # map ichi14 labels to capture24 type
    if RELABEL:
